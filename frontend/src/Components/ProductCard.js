...
const ProductCard = ({
        ...
        const checkWishlist = async () => {
            if (token && product?._id && initialIsWishlisted === undefined) {
                try {
                    ...
                    const isItemWishlisted = res.data.some(
    ...
    const handleWishlistToggle = async (e) => {
        ...
        try {
            const res = await axios.post(
                `http://localhost:5001/api/auth/wishlist/${product._id}`,
                {},
                {
                    headers: { Authorization: `Bearer ${token}` }
                }
    ...
    // ===============================
    // Image Handling (FINAL FIX)
    // ===============================
    const imageUrl = product?.image
        ? product.image   // already full URL from backend
        : itemStandard || defaultProduct;
        ...
        >
                ...
                {!isSeller && (
                    <button
                        className={`wishlist-heart-btn ${
                            isWishlisted || isWishlistPage ? 'active' : ''
                        }`}
                        onClick={handleWishlistToggle}
                        title={
                            isWishlisted || isWishlistPage
                    ...
                    >
                        <svg
                            ...
                            height="20"
                            viewBox="0 0 24 24"
                            fill={(isWishlisted || isWishlistPage) ? "#22c55e" : "none"}
                            stroke={(isWishlisted || isWishlistPage) ? "#22c55e" : "white"}
                            strokeWidth="2.5"
                            strokeLinecap="round"
            ...
            <div className="product-content">
                ...
                <div className="product-footer">
                    ...
                    <span className="seller-year-tag">
                        🎓 {product.sellerYear || "3rd Year"}
                    </span>
                </div>
